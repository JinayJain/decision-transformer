...
class DecisionTransformer(nn.Module):
    ...
    def forward(self, observations, actions, returns_to_go):
        ...
        assert returns_to_go.shape == (batch_size, seq_len)

        # embed observations, actions, returns_to_go into a shared embedding space
        obs_embed = self.obs_enc(observations)  # (batch_size, seq_len, d_model)
        action_embed = self.action_enc(actions)  # (batch_size, seq_len, d_model)
        rtg_embed = self.rtg_enc(returns_to_go)  # (batch_size, seq_len, d_model)

        # TODO: add positional encodings, by timestep

        # interleave returns_to_go, observations, actions
        x = torch.stack([rtg_embed, obs_embed, action_embed], dim=1)
        x = rearrange(x, "b k t d -> b (t k) d", b=batch_size, t=seq_len, k=3)

        # pass through transformer model
        x = self.transformer(x)  # (batch_size, 3 * seq_len, d_model)

        x = rearrange(x, "b (t k) d -> b t k d", b=batch_size, t=seq_len, k=3)
        x = x[:, :, -1, :]  # get the action hidden states
